...
def verify_prebuilt_binaries(
    ...
    settings = settings or load_settings()
    paths = prebuilt_binary_paths(settings)
    if not prebuilt_binaries_ready(settings):
        return {"ok": False, "error": "内置 ffmpeg 预编译包未安装"}
    try:
        result = subprocess.run(
        ...
        if result.returncode != 0:
            raise RuntimeError(result.stderr.strip() or "无法执行内置 ffmpeg")
        version = result.stdout.splitlines()[0] if result.stdout else "ffmpeg"
        encoders = read_ffmpeg_encoders(str(paths["ffmpeg"]))
    except Exception as exc:  # noqa: BLE001 - 启动流程需要把错误序列化
        return {"ok": False, "error": f"内置 ffmpeg 不可用：{exc}"}
    ...
    if missing:
        payload["error"] = (
            "内置 ffmpeg 缺少必需编码器：" + "、".join(missing)
        )
    return payload
...
def install_prebuilt(
    ...
    *,
    ...
    """Download and install the prebuilt ffmpeg package for this platform."""
    settings = settings or load_settings()
    key = platform_key()
    ...
    if not url:
        return {
            ...
            "error": (
                ...
                "macOS 可执行 brew install ffmpeg，或在设置页填写自定义 ffmpeg 路径。"
            ),
        }
...
def ensure_prebuilt_runtime(
    ...
    if prebuilt_binaries_ready(settings):
        ...
        logger.warning(
            "内置 ffmpeg 不可用或缺少编码器，准备重新下载预编译包：%s",
            verified.get("error"),
        )
